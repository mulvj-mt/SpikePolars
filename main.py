...
def get_schema(path_to_schema_file: str) -> pl.Schema | None:
    ...
    try:
        ...
        schema_def = {
            k: pl.String() if v == "string" else pl.Int64() for k, v in schema.items()
        }
...
def scan_csv_source(source_path: str, schema: pl.Schema | None) -> pl.LazyFrame:
    """Reads a CSV file into a Polars Lazyframe."""
    try:
        df = pl.scan_csv(
            source_path, infer_schema_length=0, separator="|", schema=schema
        )
